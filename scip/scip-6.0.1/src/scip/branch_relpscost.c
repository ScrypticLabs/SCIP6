...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   /* save bestcand and branching variable */
...
{  /*lint --e{715}*/
   ...
   SCIP_CALL( execRelpscost(scip, branchrule, lpcands, lpcandssol, lpcandsfrac, nlpcands, branchruledata->executebranch, result, &branchruledata->bestcand, &branchruledata->variable) );
...
/**! [SnippetCodeStyleParenIndent] */
   ...
   SCIP_CALL( SCIPaddBoolParam(scip, "branching/relpscost/executebranch",
         "should the branching rule branch or just be used as an oracle?",
         &branchruledata->executebranch, TRUE, FALSE,
         NULL, NULL) );
   return SCIP_OKAY;
}
   ...
   )
{
   ...
   /* execute branching rule */
   SCIP_CALL( execRelpscost(scip, branchrule, branchcands, branchcandssol, branchcandsfrac, nbranchcands, executebranching, result, &branchruledata->bestcand, &branchruledata->variable) );

   return SCIP_OKAY;
}

/** recovers candidate variables and their scores from last vanilla full strong branching call */
SCIP_RETCODE SCIPgetRelpscostData(
   SCIP*                 scip,               /**< SCIP data structure */
   ...
   )
{
   ...
   if ( variable ) 
   {
      *variable = branchruledata->variable;
   } 
