...
void SCIPremoveSibling(
   SCIP*                scip,
   SCIP_NODE*           sibling
)
{
   treeRemoveSibling(scip->tree, sibling);
}
...
void SCIPremoveChild(
   SCIP*                scip,
   SCIP_NODE*           child
)
{
   treeRemoveChild(scip->tree, child);
}

SCIP_RETCODE SCIPremoveLeaf(
   SCIP*                scip,
   SCIP_NODE*           leaf
)
{
   return SCIPnodepqRemove(scip->tree->leaves, scip->set, leaf);
}

int SCIPgetPriorityQueueSize(
   SCIP*                scip
) 
{
...
SCIP_RETCODE SCIPaddNode(
   SCIP*                scip,
   SCIP_NODE*           node
)
{
   return SCIPnodepqInsert(scip->tree->leaves, scip->set, node);
}

// END: MODIFIED BY ABHI GUPTA
